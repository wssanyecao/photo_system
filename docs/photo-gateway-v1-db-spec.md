...
> **V1.3 追加（已批准扩展）**：`file_missing_at TEXT`（NULL=本地文件在；非空=本地文件已被删除，时间戳为标记时刻）。
> 语义：`photo_assets` 记录是照片的**去重身份**（sha256 唯一），**不随文件删除而删除**；
> 本地文件缺失（Cleanup 删除 / 外部删除）时置位 `file_missing_at`：
> - 置位来源：Cleanup 真删（`cleanup --apply --yes`）按删除月份批量置位；`/photos/{id}/thumbnail|file`
>   请求发现 FILE_MISSING 时惰性置位；`maintenance --scan-missing` 对账历史/外部删除；
> - 照片页/列表（`/photos`）默认过滤 `file_missing_at IS NULL` 的资产（total 同步）；
> - 同内容（sha256 命中）重新导入缺失资产 → **复活**：新文件放回原 `archive_path`、清除
>   `file_missing_at`、asset id 不变（见 api-spec §88.5）。列由 schema migration v3 加入。
...
以下字段统一为中国上海时区语义，格式为 YYYY-MM-DD HH:MM:SS
