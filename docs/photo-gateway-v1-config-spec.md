...
# 63. 已批准配置扩展（V1.1 / V1.2）

> 本节为已批准（由项目负责人确认后并入）的规格扩展，作为 V1.0 Frozen Baseline 的补充。正文 §1-§62 语义不变；
> 实现与本节冲突时以本节为准。同 §3.3，未列出的顶层/嵌套键仍默认拒绝。
> （V1.1：§63.1 webui、§63.2 sync；V1.2：§63.3 thumbnails）

## 63.1 `webui`（可选顶层段）

用于 WebUI 展示偏好。未提供时使用默认值。

```yaml
webui:
  photos_page_size: 12     # WebUI 照片页每页张数
```

| 配置项 | 类型 | 默认值 | 校验 | 说明 |
| --- | --- | --- | --- | --- |
| `webui.photos_page_size` | int | `12` | 1..200 | 照片页默认每页张数（`GET /photos` 未显式传 limit 或 limit=0 时使用） |

## 63.2 `sync`（可选顶层段，V2）

V2 同步配置（与 `photo-gateway-v2-spec.md` §41 建议一致）。未提供 `sync` 或
`sync.enabled=false` 时，V2 Worker 不启动。

```yaml
sync:
  enabled: false                    # true 时启用 V2 Worker(周期同步)
  nas:
    host: "nas.example.local"       # NAS 主机
    ssh_port: 22                    # SSH 端口(默认 22)
    ssh_user: "photo-sync"          # NAS 同步账号
    ssh_key: "/root/.ssh/pg-key"    # SSH 私钥
    target_root: "/data/Photos"     # NAS Photo Root
  # source.root 可选：缺省由归档根推导（storage 下 Photos）
  worker:
    cycle_interval_seconds: 300     # V2 工作周期间隔(秒)
  failure:
    threshold: 3                    # 连续失败阈值(>=1)
  cleanup:
    enabled: true
    synced_retention_days: 90       # 同步成功后 R5S 保留天数(>=1)
```

校验规则：

- `sync.enabled=true` 时，`nas.{host, ssh_user, ssh_key, target_root}` 必须提供，否则启动失败；
- `ssh_port` 范围 1..65535；`worker.cycle_interval_seconds > 0`；
- `failure.threshold >= 1`；`cleanup.synced_retention_days >= 1`；
- 未知子键仍按 §3.3 拒绝。

---

## 63.3 `thumbnails`（可选顶层段；V1.2 追加）

缩略图缓存目录与过期清理规则（配套 api-spec §88.3/§88.4）。未提供本段时使用默认值，
行为与 V1.1 一致：缓存到 `<storage.root>/thumbnails/<asset_id>.jpg`。

```yaml
thumbnails:
  cache_dir: thumbnails       # 缓存目录（相对 storage.root；绝对路径亦可）
  cleanup:
    enabled: true             # 是否启用过期缓存清理
    max_age_days: 90          # 清理 mtime 超过该天数的缓存文件
```

| 配置项 | 类型 | 默认值 | 校验 | 说明 |
| --- | --- | --- | --- | --- |
| `thumbnails.cache_dir` | string | `thumbnails` | 非空 | 缩略图缓存目录；相对路径解析到 `storage.root` 之下，绝对路径直接使用 |
| `thumbnails.cleanup.enabled` | bool | `true` | — | 是否启用过期缓存清理 |
| `thumbnails.cleanup.max_age_days` | int | `90` | `>= 1` | 缓存文件 mtime 超过该天数即删除（默认 90 天） |

校验规则：

- `cache_dir` 非空；`cleanup.max_age_days >= 1`；未知子键仍按 §3.3 拒绝；
- 清理由 serve 维护任务执行：启动先清理一轮，之后每 24 小时一轮
  （与 `upload.tmp_cleanup` 同级维护，见 api-spec §88.4）；
