...
**项目名称：Photo Gateway**  
**版本：V1.1（在 V1.0 基础上含已批准行为扩展，见 §48）**  
...
> 本节为已批准（由项目负责人确认后并入）的行为扩展（V1.0 Frozen 基线的补充）。正文 §1-§47 语义不变。

## 48.1 重复上传可修正既有资产日期/归档位置

当一次上传内容与既有 `photo_assets` 完全一致（SHA-256 相同，见 §27/§42 重复判定）时，
系统不再只标记 DUPLICATE；若本次上传携带更可用的元数据，允许修正历史错误：

- 日期解析优先级与归档一致：`DateTimeOriginal → CreateDate → ModifyDate → file_mtime`；
  其中 `file_mtime` 指客户端原文件修改时间（经 `modified_ms` 重建），见 api-spec §88.1。
- 仅当**解析出的 `date_taken` 与既有资产不同**时执行修正；同日期不做无意义改写。
- 来源强度规则：新来源**不弱于**既有来源才允许覆盖
  （强度：`file_mtime < ModifyDate < CreateDate < DateTimeOriginal`）——
  例如既有资产已有 EXIF 而本次只有 mtime 时，**不覆盖** EXIF 日期。
- 目标月目录变化时，把文件从旧目录移动到正确的 `Photos/YYYY/YYYYMM`
  （目标同名冲突按 §46 自动 `_N` 递增，不覆盖）；移动失败则不改库，避免库/文件不一致。
- 修正同时更新：`date_taken/date_source/archive_path/current_filename/updated_at`；
  若既有 `exif_json` 为空而本次文件含 EXIF，一并回填。
- 原始文件内容不变；`sha256` 唯一约束不改变；DUPLICATE 记录仍完整保留。

## 48.2 归档文件 mtime 与日期、NAS 同步一致性

- Worker 在**新照片归档**及 **§48.1 修正/移动**后，将正式文件 mtime/atime 同步设置为
  `date_taken` 对应时刻（`date_taken` 无时区时按上海墙钟语义换算）。
- 目的：NAS 同步使用 `rsync -t` 保留 mtime 后，Immich 等软件在无 EXIF 资产上按 mtime 回退索引时，
  得到的是照片时间而非“写入/上传/同步”时间。
- 存量维护命令（非上传路径，人工执行）：
  - `photo_gateway align-mtime --config <cfg>`：把存量资产磁盘 mtime 对齐到 `date_taken`（差异 >1s 才改）；
  - `photo_gateway backfill-exif --config <cfg>`：为 `exif_json` 为空的存量资产补录 EXIF。
- 说明：EXIF 日期来自读操作不改原始内容；磁盘 mtime 不属于照片内容，因此不违反 §32/§33 原始文件保护。
